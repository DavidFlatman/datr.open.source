...
///         the dev_test* to stand on its own.                                  
///                                                                             
///                                                                             
...
inline void checkOption(
    dev::Test::verbosity_t& verbosity
  , const char* argv
  , const char* option
  , const dev::Test::verbosity_t& optionValue
) {
...
///         the dev::Test object.                                               
//------------------------------------------------------------------------------
typedef std::pair<int, dev::Test*> RegisterIDTest;
...
///             @li e.second.second A pointer to a dev::Test object.            
//------------------------------------------------------------------------------
...
///                                                                             
///@par Evolution                                                               
///         At first, we just had dev::Test and a main program to call all of   
///         the test individually.  All was right with the world, we just copy  
///         and pasted all the code from test harness to test harness.          
///\n\n                                                                         
///         A dev::TestSuite sprang up which was a dev::Test that ran a set     
///         of dev::Test objects (with the passed/failed stats being            
///         accumulated from the individual dev::Test objects).                 
///\n\n                                                                         
///         The duplicated code was then collected into the dev::test::classes  
///         library where the dev::Test classes could automatically register    
///         themselves with a single, *gasp* global "dev::TestSuite" (the       
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
Singleton::Singleton() : dev::Test("registered classes")
{
}
...
{
    std::cout << "dev_test_main --stats --failed\n"
...
static void displayHistory()
{
    std::cout << 
        "----------------------------------------"
        "---------------------------------------\n"
        "dev_test_main          v2.0.0           2014-02-19\n"
        "----------------------------------------"
        "---------------------------------------\n"
        "   New Features:\n"
        "       -   Added --help option.\n"
...
void Singleton::run(
  ...
  , int last
    ...
    dev::Test::verbosity_t verbosity(dev::Test::vSilent);

    for (int a=0; a < argc; ++a) {
        checkOption(verbosity, argv[a], "silent", dev::Test::vSilent);
        checkOption(verbosity, argv[a], "failed", dev::Test::vFailed);
        checkOption(verbosity, argv[a], "passed", dev::Test::vPassed);
        checkOption(verbosity, argv[a], "not-impl", dev::Test::vNotImplemented);
        checkOption(verbosity, argv[a], "header", dev::Test::vHeader);
        checkOption(verbosity, argv[a], "stats",  dev::Test::vStatistics);
        checkOption(verbosity, argv[a], "summary",dev::Test::vSummary);
        checkOption(verbosity, argv[a], "verbose",dev::Test::vVerbose);
        checkOption(verbosity, argv[a], "stamp",  dev::Test::vTimeStamp);
        checkOption(verbosity, argv[a], "time",   dev::Test::vRunLevelTime);
        checkOption(verbosity, argv[a], "slow",   dev::Test::vRunLevelTimeLong);
...
void run(
    int argc
  , char** argv
  , dev::Test::OutputFunction outputFunc
  , const std::string& indentLevel
  , int first
  , int last
) {
...
///         In our world (dev::Test) it is normal for someone to create a       
